...
interrupt_list = loader.interrupt_loader() # List of interrupts in the interrupts folder
...
def open_show():
    ...
    show_directory = fd.askdirectory(initialdir = "/home/pi", title="Please select the show folder") #Presents a diolog box to select a folder and save the directory to a variable
    ...
    loader.show_loader(show_directory) #Calls the load function in show_load.py to initialize the show

# Command for the back button
def back_command():
    controller.back() #Call the cue_backward function in show_control.py
    cue_number.clear() #Clears the cue text
    cue_number.append("Cue: " + str(controller.current_cue)) #Changes the cue text to current cue number

# Command for the forward button, same as the back button but calls the forward command instread
def forward_command():
    controller.forward()
    cue_number.clear()
    cue_number.append("Cue: " + str(controller.current_cue))
...
def send_interrupt():
    ...
    controller.interrupt(selected_interrupt) #calls the interrupt_show function in show_control.py with the selected_interrupt as the parameter
...
interrupt_stop = PushButton(interrupt_box, text="Stop Interrupt", command=controller.set, align="left")
#debug_button = PushButton(app, text="Debug", command=debug)
